...
def classify_batch(
    ...
    for attempt in range(num_attempts):
        ...
        for retry in range(llm_retries):
            ...
            try:
                ...
                print(
                    ...
                    f"retry {retry + 1}/{llm_retries}): {e}"
                )

                if retry < llm_retries - 1:
                    ...
                    print(
                        "All retries failed for this "
                        "classification attempt."
                    )

        # ========================================================
        # IF ALL LLM RETRIES FAILED
        # ========================================================

        if result is None:

            # Don't add anything to all_results.
            # The consensus section will detect that
            # this attempt has no prediction.

            continue

        parsed_results = parse_response(
            result,
            texts
        )

        validated_results = validate_results(
            parsed_results
        )
    ...
    for text in texts:
        ...
        # --------------------------------------------------------
        # Get the prediction for this text from each attempt
        # --------------------------------------------------------

        for attempt_results in all_results:
        ...
        # --------------------------------------------------------
        # Not all attempts returned a prediction
        # --------------------------------------------------------

        if len(predictions) != num_attempts:

            # Use the first available result as the base
            if predictions:

                final_result = predictions[0].copy()
            ...
            final_result["label"] = "Ambiguous"
            final_result["parent_label"] = "Ambiguous"
        ...
        # --------------------------------------------------------
        # Get labels from the 3 attempts
        # --------------------------------------------------------
